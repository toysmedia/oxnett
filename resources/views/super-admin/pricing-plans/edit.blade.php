...
@section('title', 'Edit ' . $pricingPlan->name)
...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Edit: {{ $pricingPlan->name }}</li>
    </ol>
</nav>
    ...
    <div class="col-lg-9">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-0 pt-3 pb-0">
                <h6 class="mb-0 fw-semibold"><i class="bi bi-pencil me-2 text-warning"></i>Edit: {{ $pricingPlan->name }}</h6>
            </div>
            <div class="card-body">
                @if($errors->any())
                ...
                <form method="POST" action="{{ route('super-admin.pricing-plans.update', $pricingPlan) }}">
                    @csrf
                    @method('PUT')
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Plan Name <span class="text-danger">*</span></label>
                            <input type="text" name="name" value="{{ old('name', $pricingPlan->name) }}" class="form-control @error('name') is-invalid @enderror" required>
                            @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Price (KES) <span class="text-danger">*</span></label>
                            <input type="number" name="price" value="{{ old('price', $pricingPlan->price) }}" class="form-control @error('price') is-invalid @enderror" min="0" step="0.01" required>
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="billing_cycle" class="form-select @error('billing_cycle') is-invalid @enderror" required>
                                <option value="monthly" @selected(old('billing_cycle', $pricingPlan->billing_cycle) === 'monthly')>Monthly</option>
                                <option value="quarterly" @selected(old('billing_cycle', $pricingPlan->billing_cycle) === 'quarterly')>Quarterly</option>
                                <option value="yearly" @selected(old('billing_cycle', $pricingPlan->billing_cycle) === 'yearly')>Yearly</option>
                            </select>
                            @error('billing_cycle')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Max Customers</label>
                            <input type="number" name="max_customers" value="{{ old('max_customers', $pricingPlan->max_customers) }}" class="form-control" min="0" placeholder="Leave blank for unlimited">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Max Routers</label>
                            <input type="number" name="max_routers" value="{{ old('max_routers', $pricingPlan->max_routers) }}" class="form-control" min="0" placeholder="Leave blank for unlimited">
                        </div>
                        <div class="col-md-4 d-flex align-items-end">
                            <div class="form-check form-switch mb-2">
                                <input class="form-check-input" type="checkbox" name="is_active" value="1" id="isActive" @checked(old('is_active', $pricingPlan->is_active))>
                                <label class="form-check-label" for="isActive">Plan is Active</label>
                            </div>
                        </div>
                        <div class="col-12">
                            <label class="form-label">Description</label>
                            <textarea name="description" rows="2" class="form-control">{{ old('description', $pricingPlan->description) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive mb-2">
                        <table class="table table-sm align-middle" id="featuresTable">
                            ...
                            @foreach($pricingPlan->featureFlags as $flag)
                            <tr class="feature-row">
                                <td>
                                    <input type="text" class="form-control form-control-sm feature-key" value="{{ $flag->feature_key }}" placeholder="e.g. reports" data-key="{{ $flag->feature_key }}">
                                </td>
                                <td>
                                    <input type="text" name="feature_flags[{{ $flag->feature_key }}]" value="{{ old("feature_flags.{$flag->feature_key}", $flag->feature_name) }}" class="form-control form-control-sm feature-name" placeholder="e.g. Reports Module" required>
                                </td>
                                <td>
...
@push('scripts')
<script>
let featureIndex = {{ $pricingPlan->featureFlags->count() }};
