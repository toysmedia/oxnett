...
<div class="row">
    ...
    <div class="col-sm-12">
        ...
        <form action="{{ route('admin.isp.resellers.store') }}" method="POST">
            @csrf
            <div class="row">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                       value="{{ old('name') }}" placeholder="e.g. Jane Reseller" required>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
                                       value="{{ old('email') }}" placeholder="reseller@example.com">
                                @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Location / Area</label>
                                <input type="text" name="location" class="form-control @error('location') is-invalid @enderror"
                                       value="{{ old('location') }}" placeholder="e.g. Westlands, Nairobi">
                                @error('location')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Notes</label>
                                <textarea name="notes" rows="3" class="form-control @error('notes') is-invalid @enderror"
                                          placeholder="Optional notes about this reseller">{{ old('notes') }}</textarea>
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" name="username" class="form-control @error('username') is-invalid @enderror"
                                       value="{{ old('username') }}" placeholder="Login username" required>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-text">Percentage discount on package prices for this reseller.</div>
                                ...
                                <select name="router_ids[]" class="form-select @error('router_ids') is-invalid @enderror" multiple>
                                    @foreach($routers ?? [] as $router)
                                        <option value="{{ $router->id }}"
                                            {{ in_array($router->id, old('router_ids', [])) ? 'selected' : '' }}>
                ...
                <div class="col-sm-12">
                    <button type="submit" class="btn btn-primary me-2">
                        <i class="bx bx-save me-1"></i> Save Reseller
                    </button>
