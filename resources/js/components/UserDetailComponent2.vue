                                ...
                                <div class="col-sm-3 mt-4">
                                    <div class="card overview-card">
                                        <div class="card-body p-2">
                                            ...
                                            <p class="card-title mb-0 text-end"> {{ currency+getPackagePrice() }}  <small>/ {{ getPackageValidity() }}</small></p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="card" style="border-top: 2px solid #696cff6b;">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div v-if="duration.expire_at" class="row mb-6">
                                                ...
                                                <div class="col-sm-8">
                                                    <p id="duration" class="pt-2 text-dark">{{ duration.start_at }} <span class="text-dark fs-5">～</span> {{ duration.expire_at }}</p>
                                            ...
                                            <div class="row">
                                                <div class="col-sm-8 offset-sm-4">
                                                    <button @click="payBillConfirm" type="button" class="btn btn-primary btn-save ms-5" :disabled="getPackagePrice() == -1 || (getCost() > parseInt(user_info.seller?.balance))"><i class='bx bx-paper-plane me-1' ></i> Pay Now</button>
                                                </div>
                                            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="d-grid gap-5 col-sm-4 mx-auto">
                                <button v-if="0" @click="othersApiConfirm('synchronize')" class="btn btn-outline-primary" type="button">Synchronize</button>
...
export default {
    ...
    methods: {
        ...
        payBill: function () {
            const self = this;
            loading();
            let url = BASE_URL + '/seller/payments/pay-bill';
            const data = {
        ...
        getPackageValidity: function () {
            const index = this.packages.findIndex(item => item.id === this.bill.package_id);
            return index > -1 ? this.packages[index].valid : '';
        },
        getCost: function () {
            const index = this.packages.findIndex(item => item.id === this.bill.package_id);
        ...
        fetchUser: function () {
            const self = this;
            loading();
            axios.get(BASE_URL + '/seller/users/' + self.user_id + '/fetch')
                .then((response) => {
        ...
        updateApi: function (action) {
            const self = this;
            loading();
            let url = BASE_URL + '/seller/users/' + this.user_id + '/update-api/';
        ...
        othersApi: function (action) {
            const self = this;
            loading();
            let url = BASE_URL + '/seller/users/' + self.user_id + '/others-api/';
            axios.post(url + action)
                .then((response) => {
                    notify(response.data.message, 'success');
                    if(action == 'delete' || action == 'delete-with-mikrotik') {
                        location.href = BASE_URL + '/seller/users';
                    }
                })
                .catch(error => {
                    notify(error.response.data.message, 'error');
        ...
        checkMikrotikStatus: function () {
            const self = this;
            loading();
            let url = BASE_URL + '/seller/users/' + self.user_id + '/server-pppoe-status';
            axios.get(url)
                .then((response) => {
        ...
        seeAllPaymentsLink: function () {
            return BASE_URL + '/seller/payments?user_id='+this.user_id;
        },
        ...
        applyGrace: function () {
            loading();
            let url = BASE_URL + '/seller/payments/grace-payment';
            const data = {
